...
sub _start_server ( $port, $timeout, $debug, $cleanup ) {
    ...
    return _start_server_windows( $port, $timeout, $debug, $cleanup ) if IS_WIN;
...
sub _start_server_windows ( $port, $timeout, $debug, $cleanup) {
        ...
        my $cmdstring = join(' ', @cmdprefix, qq{"$node_bin"}, qq{"$server_bin"}, "--port", $port, $debug );
        print "$cmdstring\n" if $debug;
        system($cmdstring);
        _wait_port( $port );
        return $pid;
